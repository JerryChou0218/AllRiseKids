...
## Framework Rules (Milestone-Driven Agent Framework v2.0)
1. **State**: 動作前先讀 `milestone-state.json`，它是唯一事實來源。
2. **Context**: 只載入「當前里程碑 spec ＋ 前一個里程碑結果摘要 ＋ 本檔規則」，不重播完整歷史。
3. **Budget**: 每個耗費階段前檢查預算，達上限即停。
4. **Retry**: 每個里程碑最多 3 次，第 3 次失敗 → 設 `blocked` 停下等人工。
5. **Rollback**: 驗證失敗時 `git stash` 還原工作區再重新規劃。
...
## Current Milestone
讀 `milestone-state.json` 的 `current_milestone_id`（目前為 7，前 6 個已完成）。
