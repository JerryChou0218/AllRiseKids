...
## Current Milestone
讀 `milestone-state.json` 的 `current_milestone_id`（目前為 16，已 `complete`）。M1–M16 除 M9（帳號制雲端同步，使用者主動暫緩）外皆 `complete`；M9 維持 `pending`，日後要做雲端可直接續接。M16＝GitHub + Vercel 部署上線（`https://allrisekids.vercel.app`）＋ PWA 安裝 QR code（`install.html`），之後 `git push` 會自動觸發 Vercel 重新部署。
