...
document.getElementById('tf-completion').value='餵完飼料並補滿水。';
...
editingTaskId=null;
saveTaskForm();
assert(state.taskPool.some(t=>t.name==='餵狗' && t.custom && t.completion==='餵完飼料並補滿水。'), '家長新增含完成條件的自訂任務到任務池');
...
assert(nrTask.completion==='餵完飼料並補滿水。', '每日任務實例保留完成條件');
