...
parentTestChild(A.id);
parentOpenPage('skills');
__assert(parentUnlocked && currentScreen==='skills', '家長管理/測試模式可直接查看技能頁');
parentOpenPage('inventory');
__assert(currentScreen==='inventory', '家長管理/測試模式可直接查看物品頁');
parentOpenPage('shop');
__assert(currentScreen==='shop' && shopTab==='virtual', '家長管理/測試模式可直接查看虛擬商店');
parentTestChild(B.id);
__assert(parentUnlocked && cloudStore.getSession().activeChildId===B.id && state.player.name==='小美', '家長管理/測試模式可直接切換孩子');
parentBackHome();
__assert(cloudStore.getSession().activeChildId===null, '家長管理/測試模式可回家長首頁');
exitChild();
enterChild(A.id);
